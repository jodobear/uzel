...
| Canonical graph current | `graphify update .`; obsolete nodes absent; `WORKFLOW.md` and current plan nodes present | PASS |

## Local gate

`pnpm docs:check` under the locked Nix environment passed with zero errors and warnings.
...
Merge requires required CI, GitHub Codex, and CodeRabbit green on one exact SHA, followed by
one final affected validation on that SHA. Any semantic change invalidates prior review.
