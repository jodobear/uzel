...
- Stop for Prompt 02 review: after freezing and pushing the exact PR SHA, run local CodeRabbit's required candidate-review mode and GitHub Codex concurrently or serially; acceptance waits for both dispositions, while only a recorded pre-finding `rate_limit` from that CodeRabbit mode activates the approved fallback. Revise through `$gsd-plan-phase 1 --reviews` as required; do not execute in review session.
