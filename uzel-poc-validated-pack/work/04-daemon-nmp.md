...
- Add a bounded chunked verified-asset transfer (or equivalently bounded private custom-scheme stream) from daemon to trusted Tauri host. Keep a finite control-frame ceiling with explicit runtime-envelope JSON-escaping headroom, reject out-of-order/oversized transfers, and never expose an artifact filesystem path to WebKit.
