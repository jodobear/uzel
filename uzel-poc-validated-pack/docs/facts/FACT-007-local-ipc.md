...
- **Decision:** Use AF_UNIX at `$XDG_RUNTIME_DIR/uzel/napd.sock`, protocol version 0, bounded length-prefixed JSON, one POC client, and typed operation/error enums. The 4,096-byte probe established feasibility, not the permanent product ceiling; Slice 04 later established 524288 bytes with explicit runtime-envelope escaping headroom. Do not create TCP, remote access, or a public compatibility protocol.
