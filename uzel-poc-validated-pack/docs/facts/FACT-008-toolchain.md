...
- **Decision:** Commit a flake/devShell and all language locks in Slice 01. Use `nix develop`, not `nix shell`, for pkg-config setup; expose Corepack's pnpm shim on `PATH`; use Fallow's `ignorePatterns` field.
