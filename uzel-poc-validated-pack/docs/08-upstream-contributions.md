...
| `pablof7z/nampplets` | `jodobear/nampplets:feat/identity-cache-refresh` | `b5a036e119aa0163f35baea1a1ee7952ffbf5865` | retains a configured relay observation after an empty cache frame; event-driven cancellation replaces 50 ms polling; terminal empty evidence returns immediately; author-bearing identity reads use operator public lanes; `identity.rs` is 296 lines and dedicated `identity_refresh.rs` is 197; file-growth ratchet, 18 Python conformance tests, baseline verification, fmt, workspace all-target Clippy, and full workspace tests pass | [jodobear fork PR #4](https://github.com/jodobear/nampplets/pull/4), open; fresh exact-head Codex review requested | Uzel pins compatibility branch `jodobear/nampplets:uzel/identity-refresh-compat-dd64` at `d533a63d519c14470f900323958509cdea1c6479`; advance only after fresh review plus exact identity, follows, and picture probes pass |
