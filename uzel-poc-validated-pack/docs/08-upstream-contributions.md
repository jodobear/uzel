...
| `pablof7z/nampplets` | `jodobear/nampplets:feat/trusted-shell-multi-surface` | `7eccdee76a1afcfc3ff026c8f41b0072a4601840` | focused trusted-shell Node tests; legacy host 4/4 and package 4/4; 22 Python conformance tests; 52-file/211-envelope/10-falsifier baseline; validator, digests, fingerprint, file-growth, AntiSlop, Rust workspace and bindings CI; Codex exact-head review clean after remount-bound and Apple test-path fixes; Uzel two-surface Fedora/WebKit smoke green | [jodobear fork PR #1](https://github.com/jodobear/nampplets/pull/1), open and mergeable; all Uzel-relevant checks green, Apple package job still running at 2026-07-29; cross-owner PR creation blocked by current fine-grained PAT | Uzel is pinned to the exact reviewed commit; merge the fork PR after its full CI settles, then submit a clean cross-owner upstream PR when credentials permit |
