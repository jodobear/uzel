...
hardening/graph chain: 27986c0 -> d80efd4 -> 8df9916 -> 61350a3 -> ed7ba54 -> 8860a6f -> d138b7a -> c379448 -> f9323bb -> 5c45fa6 -> c133560 -> 624b235 -> 1611dcb -> 97a0232 -> 8423569 -> 040fdb6 -> b015454 -> 777bd30 -> 5045532 -> d8301c1 -> 8b1552d -> b09ac73 -> 96798a6 -> f2b2ed3 -> a8b65a4 -> 079b050 -> 3fddd13 -> b6a4a29 -> e6b4840 -> 5091bca -> ed1efb4 -> c5f24a4 -> 9847273
current review fix: before STRUCTURE_OK, the shell independently authenticates the complete canonical lock digest, binds each retained event text to its audited event ID, and completes bounded observable cleanup; the trust-result jq emits only a fixed token through its 1,024-byte boundary, while the original typed Node record is separately capped at 16,384 bytes, required to be one physical line, and preserved exactly for trust exit 2; a 1,500-character diagnostic retains code invalid-lock and an over-cap record fails infrastructure; entry name and naddr jq stdout/stderr have independent 1,024-byte ceilings, reject multiple lines, CR/LF, NUL suffixes, unknown names, and malformed naddr scalars before any nak data operation; temporary storage begins with one bounded `/tmp/uzel-external-corpus.*` proposal that must not exist, then an exclusive mode-700 owned directory and five noclobber children whose globals become cleanup-owned only after successful creation; pre-existing file, directory, and inserted-child sentinels remain unchanged; entry enumeration and event extraction also use finite files; eventText remains outside the canonical lock digest so harmless signed-event JSON whitespace stays accepted; use git rev-parse HEAD after commit for the exact documentation object ID because a commit cannot embed its own resulting SHA
exact-head commands: node --test scripts/verify-external-napplet-corpus.test.mjs; nix --extra-experimental-features 'nix-command flakes' develop -c bash scripts/verify-external-napplet-corpus.test.sh; nix --extra-experimental-features 'nix-command flakes' develop -c bash scripts/verify-external-napplet-corpus-lock.test.sh; nix --extra-experimental-features 'nix-command flakes' develop -c bash scripts/verify-external-napplet-corpus.sh; shellcheck --severity=warning scripts/verify-external-napplet-corpus.sh scripts/verify-external-napplet-corpus.test.sh scripts/verify-external-napplet-corpus-lock.test.sh; corepack pnpm docs:check; git diff --check
observable result: Node verifier tests pass, including rejection of unsigned top-level event members, symlinked lock, FIFO, directory/ENOTDIR evidence, pre-stat and grow-after-stat oversize evidence, exact read-error classification, and hostile LF/CR diagnostic encoding; shell classification reports EXTERNAL_NAPPLET_CORPUS_CLASSIFICATION_TEST_OK trust=2 infrastructure=3 version=pinned setup=private-exclusive node=typed-bounded trust-result=bounded-preserved jq=bounded entry-scalars=exact-bounded nak=bounded event-id=bound event-whitespace=allowed snapshot=exact-four snapshot-cap=derived subprocess=stream-bounded size-probe=builtin cleanup=owned-bounded-observable; it includes harmless whitespace around all four signed event texts, a valid four-event near-limit snapshot above 65,536 bytes, exact 1,500-character trust preservation, over-cap trust rejection, continuous trust-result/metadata jq stdout rejection, name/naddr jq stdout and stderr rejection, no nak verify/decode after scalar failure, NUL/multiple scalar rejection, bounded mktemp output, unchanged pre-existing file/directory/inserted-child sentinels, ignored false/hanging/noisy retired wc overrides, and a self-consistent zero-entry rejection; canonical lock regression reports EXTERNAL_NAPPLET_CORPUS_LOCK_TEST_OK digest=canonical-complete mutations=4 sha256sum=bounded-classified; full pinned verifier reports STRUCTURE_OK and CORPUS_OK for four entries at source aa4dc7a0799d95e3066b50055b29685d6e376045; ShellCheck warning gate, documentation audit, and diff check pass
verified behavior: four hzrd149 coordinates retain exact signed events, author, kind, d tag, naddr/relay hints, path digest and byte length, aggregate, audited domains, server list, source license URL, audit date, publication time, and exact trust/infrastructure policy; each retained signed event contains exactly the seven Nostr event keys, so unsigned top-level extensions fail trust; exact flake nak 0.20.1 verifies every event hash/signature and naddr offline; a four-entry exact-schema immutable verified snapshot crosses into nak only after the shell authenticates the full canonical lock digest and event-ID binding independently of Node; eventText remains excluded so harmless signed-event whitespace is accepted; invalid Node evidence exits trust 2 only through one exact-schema, one-line JSON record capped at 16,384 bytes, while its jq validator can emit only the fixed token within 1,024 bytes and the original encoded diagnostic is preserved; successful Node output must contain exactly the four audited names and entry copies before processing; Node snapshot stdout/stderr each have a derived 262,144-byte streaming ceiling, canonical and entry-enumeration jq output are capped at 65,536 and 262,144 bytes, trust validation, snapshot metadata, event binding, and each entry scalar at 1,024 bytes, verified event extraction at 16,384 bytes, sha256sum at 1,024 bytes, and nak version/decode at 65,536 bytes per stream; Bash ulimit bounds files before ingestion and bounded reads reject NUL or extra-line suffixes; lock and event paths are opened O_NOFOLLOW and O_NONBLOCK, checked for canonical containment and exact identity through /proc/self/fd, fstat-checked as regular files, and read through fixed 16,385-byte buffers that reject more than 16,384 evidence bytes without unbounded allocation; temporary cleanup targets only the exact private directory and child paths whose exclusive creation succeeded; path/type errors EISDIR, ENOTDIR, ELOOP, and ENXIO are trust failures, while operational EACCES and EIO remain infrastructure failures; missing, wrong-version, timed-out, crashed, malformed-output, untyped-exit, or oversized-output tooling exits infrastructure 3; no artifact HTML, runtime registration, UI, pin, or dependency changed
