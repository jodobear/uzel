...
current review fix: the Node stream ceiling is derived as 262,144 bytes from four 2x-expanded 16,384-byte event texts, two 2x lock/entry metadata allowances, and 65,536 bytes of envelope headroom; a valid four-event near-limit snapshot above the retired 65,536-byte cap completes full verification; nak version/decode retain 65,536-byte per-stream ceilings; external wc probing is removed in favor of Bash built-in file tests, with false, hanging, and noisy retired overrides proving they cannot cause raw exits; every temporary file created before any of the five possible setup failures is asserted absent; focused regressions and this status travel in one commit; use git rev-parse HEAD after commit for its exact object ID because a commit cannot embed its own resulting SHA
