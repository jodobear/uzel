...
hardening/graph chain: 27986c0 -> d80efd4 -> 8df9916 -> 61350a3 -> ed7ba54 -> 8860a6f -> d138b7a -> c379448 -> f9323bb -> 5c45fa6 -> c133560 -> 624b235 -> 1611dcb -> 97a0232 -> 8423569 -> 040fdb6 -> b015454 -> 777bd30 -> 5045532 -> d8301c1 -> 8b1552d -> b09ac73 -> 96798a6 -> f2b2ed3 -> a8b65a4 -> 079b050 -> 3fddd13
current review fix: before STRUCTURE_OK, the shell independently authenticates the complete canonical lock digest, binds each retained event text to its audited event ID, and completes bounded observable cleanup; snapshot-metadata jq stdout and stderr now cross the existing finite-file subprocess boundary with a 1,024-byte ceiling instead of accumulating in Bash command-substitution memory; the Node verifier rejects any signed event whose top-level keys are not exactly content, created_at, id, kind, pubkey, sig, and tags; focused regressions prove continuous metadata jq output is bounded and an unsigned injected event member fails trust; eventText remains outside the canonical lock digest so harmless signed-event JSON whitespace stays accepted; use git rev-parse HEAD after commit for the exact documentation object ID because a commit cannot embed its own resulting SHA
