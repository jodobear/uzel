...
hardening/graph chain: 27986c0 -> d80efd4 -> 8df9916 -> 61350a3 -> ed7ba54 -> 8860a6f -> d138b7a -> c379448 -> f9323bb -> 5c45fa6 -> c133560 -> 624b235 -> 1611dcb -> 97a0232 -> 8423569 -> 040fdb6 -> b015454 -> 777bd30 -> 5045532 -> d8301c1 -> 8b1552d -> b09ac73 -> 96798a6 -> f2b2ed3 -> a8b65a4 -> 079b050 -> 3fddd13 -> b6a4a29 -> e6b4840 -> 5091bca
current review fix: before STRUCTURE_OK, the shell independently authenticates the complete canonical lock digest, binds each retained event text to its audited event ID, and completes bounded observable cleanup; both Node trust-result validation and snapshot-metadata jq stdout/stderr now cross the existing finite-file subprocess boundary with separate 1,024-byte ceilings instead of accumulating in Bash command-substitution memory; the Node verifier rejects any signed event whose top-level keys are not exactly content, created_at, id, kind, pubkey, sig, and tags; focused regressions prove continuous trust-result and metadata jq output are bounded and an unsigned injected event member fails trust; eventText remains outside the canonical lock digest so harmless signed-event JSON whitespace stays accepted; use git rev-parse HEAD after commit for the exact documentation object ID because a commit cannot embed its own resulting SHA
...
observable result: Node verifier tests pass, including rejection of unsigned top-level event members, symlinked lock, FIFO, directory/ENOTDIR evidence, pre-stat and grow-after-stat oversize evidence, exact read-error classification, and hostile LF/CR diagnostic encoding; shell classification reports EXTERNAL_NAPPLET_CORPUS_CLASSIFICATION_TEST_OK trust=2 infrastructure=3 version=pinned setup=checked node=typed-bounded trust-result=bounded jq=bounded nak=bounded event-id=bound event-whitespace=allowed snapshot=exact-four snapshot-cap=derived subprocess=stream-bounded size-probe=builtin cleanup=bounded-observable, including harmless whitespace around all four signed event texts, a valid four-event near-limit snapshot above 65,536 bytes, rejection of continuous Node trust-result and snapshot-metadata jq stdout before any typed trust result or STRUCTURE_OK, ignored false/hanging/noisy retired wc overrides, rejection of a self-consistent zero-entry snapshot and continuous Node/version/decode stdout or stderr, and complete prior-file cleanup at setup failures one through five; canonical lock regression reports EXTERNAL_NAPPLET_CORPUS_LOCK_TEST_OK digest=canonical-complete mutations=4 sha256sum=bounded-classified; full pinned verifier reports STRUCTURE_OK and CORPUS_OK for four entries at source aa4dc7a0799d95e3066b50055b29685d6e376045; ShellCheck warning gate, documentation audit, and diff check pass
verified behavior: four hzrd149 coordinates retain exact signed events, author, kind, d tag, naddr/relay hints, path digest and byte length, aggregate, audited domains, server list, source license URL, audit date, publication time, and exact trust/infrastructure policy; each retained signed event contains exactly the seven Nostr event keys, so unsigned top-level extensions fail trust; exact flake nak 0.20.1 verifies every event hash/signature and naddr offline; a four-entry exact-schema immutable verified snapshot crosses into nak only after the shell authenticates the full canonical lock digest and event-ID binding independently of Node; eventText remains excluded so harmless signed-event whitespace is accepted; invalid Node evidence exits trust 2 only through one exact-schema JSON result whose controls remain encoded and whose validating jq has a 1,024-byte stdout/stderr ceiling, while nak uses its typed result; successful Node output must contain exactly the four audited names and entry copies before processing; Node stdout and stderr each have a derived 262,144-byte streaming ceiling, canonical jq output is capped at 65,536 bytes, trust-result, snapshot-metadata, and event-binding jq output at 1,024 bytes each, sha256sum output at 1,024 bytes, and nak version/decode at 65,536 bytes per stream; Bash ulimit bounds the files before ingestion and built-in -s checks empty-stream contracts without an external size subprocess; lock and event paths are opened O_NOFOLLOW and O_NONBLOCK, checked for canonical containment and exact identity through /proc/self/fd, fstat-checked as regular files, and read through fixed 16,385-byte buffers that reject more than 16,384 evidence bytes without unbounded allocation; path/type errors EISDIR, ENOTDIR, ELOOP, and ENXIO are trust failures, while operational EACCES and EIO remain infrastructure failures; missing, wrong-version, timed-out, crashed, malformed-output, untyped-exit, or oversized-output tooling exits infrastructure 3; all five temporary files are checked and cleaned if any creation fails or before success; no artifact HTML, runtime registration, UI, pin, or dependency changed
