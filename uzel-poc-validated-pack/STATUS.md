...
hardening/graph chain: 27986c0 -> d80efd4 -> 8df9916 -> 61350a3 -> ed7ba54 -> 8860a6f -> d138b7a -> c379448 -> f9323bb -> 5c45fa6 -> c133560 -> 624b235 -> 1611dcb -> 97a0232 -> 8423569 -> 040fdb6 -> b015454
current review fix: successful Node snapshots must match the exact shell interface schema, contain the four audited entry names in fixed order, and duplicate the same lock entries before STRUCTURE_OK or nak processing; Node verification, nak --version, and every nak decode stream stdout and stderr through kernel-limited files with an accepted 65,536-byte cap per stream, so continuous output cannot grow a command substitution or temporary file without bound; deterministic zero-entry and continuous stdout/stderr regressions and this status travel in one commit; use git rev-parse HEAD after commit for its exact object ID because a commit cannot embed its own resulting SHA
