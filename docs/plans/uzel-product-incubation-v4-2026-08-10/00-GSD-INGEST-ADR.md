# Uzel product-incubation v4 GSD ADR projection
...
- D-26 — Plan `01-05` owns exact source-pin ecosystem verification, immutable upstream registry and compatibility profile, package/profile-hash binding plan, fail-before-guest capability negotiation and transcript vectors, SIR, interop/version-skew matrix, clean-room fixture and independent-peer gate, maturity ledgers, upstream/local-patch records that separate merge/release/Uzel-adoption/patch-removal states, dedicated upstream fork/worktree/contribution-policy process, terminology/decision/learning/education records, admission/fairness baseline, and read-only upstream radar. Plans `01-06` and `01-07` are bounded execution-only decompositions of D-31 closeout: `01-06` produces and freezes public projection/evidence candidate E under normal task-atomic commits; `01-07` produces P in a later task commit, performs exact-P review when approval is sought, stops for the pure human decision checkpoint, writes the bound decision record, and preserves the distinct final-SHA review condition. They add no outcome or scope beyond authoritative `00-GSD-INGEST.md`.
- D-27 — Execute `01-01` through `01-07` sequentially in the human-created Phase 1 worktree with the strict chain `01-01 -> 01-02 -> 01-03 -> 01-04 -> 01-05 -> 01-06 -> 01-07`; replay may use one separate disposable checkout.
...
- D-31 — Prompt 02 review against the exact plan head is mandatory after planning: after committing and pushing the immutable candidate, local CodeRabbit's required candidate-review mode may run while GitHub Codex reviews the exact pushed PR SHA; acceptance waits for both dispositions. If that local CodeRabbit mode returns a recorded `rate_limit` error before findings, green GitHub Codex satisfies the review gate; no later, alternate-mode, or other CodeRabbit failure does. Any later commit invalidates review evidence. No other AI reviewer is part of this programme, no Phase 1 implementation occurs before the applicable review path passes, and after reading the Phase 1 evidence matrix a human must explicitly approve Phase 2 before it begins.
...
- `01-06`: project/reconcile complete evidence and freeze immutable evidence candidate E using task-atomic commits.
- `01-07`: create packet candidate P from E, perform the approval review gate, stop for the human transition decision, and preserve the distinct exact-final-SHA gate.
- Stop for independent Prompt 02 review using only local CodeRabbit followed by GitHub Codex on the exact candidate SHA; revise through `$gsd-plan-phase 1 --reviews` as required; do not execute in the review session.
...
- Generated Phase 1 context names both evidence tracks, all seven bounded plans, `b185ad1`, immutable profile/registry work, manifest-identity gate, fairness ownership, the approved review and rate-limit fallback, and human stop before Phase 2.
